            ...
            if len(self.food) != 0:
                if foodid in self.food.keys():
                    ...
                    if x == '1':
                        ...
                        self.food[foodid][
                            'NAME'] = new_name  # {'NAME':name,'QUANTITY':quantity,'PRICE':price,'STOCK':stock}
            ...
            if len(self.food) == 0:
                ...
                while True:
                    ...
                    for i in self.food.keys():
                        print(
                            f"FOOD ID {i} : [{self.food[i]['NAME']} {self.food[i]['PRICE']} {self.food[i]['QUANTITY']}] (Availabe Items Left : {self.food[i]['STOCK']})")
                    print('\n1. ORDER\n2. EXIT')
                    x = input('\nEnter the option : \n')
                    if x == '1':
